...
def common_multi_insert(cur, tablename, fieldnames, dic_list_al):     #   cur 连接，表明，更新的属性，我们构造好的list
    try:
...
def update(cur,att,dict):   #采用表更新，速度更快
    sql_str=" Drop table if exists temp"  #建立temp表时，如果存在删除
    cur.execute(sql_str)
    sql_str = "CREATE TABLE temp(days INT NOT NULL, id INT NOT NULL, %s FLOAT NULL,PRIMARY KEY ( days,id )) "%(att)#建立temp表
    cur.execute(sql_str)
    cur.execute("ALTER TABLE `temp` ENGINE = MYISAM ")
    common_multi_insert(cur239, "temp", ['days', 'id', att], dict)
    for year in range(1990,2018,1):
        tablename = "select_stock_filter_%d"%year  #对所有的表依次更新
...
def beyond_year(day1,day2,id,att):   #it just for test ,don't need
    if day2-day1>=365:
        print "it beyond one year,day1=%d,day2=%d,diff=%d,id=%d,att=%s"%(day1,day2,day2-day1,id,att)

def return_index(day):    #股票时间并不是连续的，我们date_list 是基准,返回day在datelist中的位置
...
def deal_sql(list_att):
    dict_all_zgb={}  # it is need to be save for next step(每股='mg')
    for key in list_att:
        ...
        for id, update_day, value in cur239.fetchall():    #get data one line by one line,it is a touple
        ...
        for secu_id in secu_list:
            if secu_id in dict_121:            #有的股票某个属性全都为空
                ...
                for index,up_day in enumerate(list_update):  #get update and index at same time
                    if index==len(list_update)-1:
                        day_start=up_day
                        year = 2017  # last year
                        day_end=y2m[year][1]
                        Add_list(dict_list_all, day_start,day_end,secu_id,key,dict_221[(secu_id,up_day)],dict_all_zgb) # dict_list_all is a list be made up dict,every dict have 3 key ,it is used for update
...
def Add_list(dict_list_all,day_start,day_end,id_value,name,value,dict_all_zgb):  #it is for dict_list_all
    if day_start<y2m[2017][1]:    #17378=2017-7-31 end_date
        index1=return_index(day_start)+1  #update_day +1 更新数据
        ...
        if name[0] != 'm'or (id_value,date_list[index2]) not in dict_all_zgb:  # repsent mg and zgb is NULL
            for days in date_list[index1:index2 + 1]:
                one_dict={'days':days,'id':id_value,name:value}  #sql 语句的两个primekey 以及我们更新的值
                    ...
                    if start_zgb==end_zgb:     #same ,don't need update
                        ...
                        for days in date_list[index1:index2 + 1]:
                            ...
                            if start_zgb!=days_zgb and days_zgb is not None:
                                value_temp=value*days_zgb/start_zgb    #在'mg'更新区间内,zgb 发生变化，我们需要对‘mg’进行更新
                                start_zgb=days_zgb
...
if __name__ == "__main__":
    start = time.clock()
    cnn = MySQLdb.connect(host='192.168.1.239', user='jgp', passwd='123456', db='com_stock', charset='utf8')  #connect sql
    cur239 = cnn.cursor()
    ...
    for year in range(1990,2018,1):     # Set NULL about spent 30 sec
        ...
        expression=one_row_val_sql%('ltg','zgb','yg_rate','mgsy','mgjzc','jlr','jlrtb_rate','ystb_rate','jzcsy_rate','jzcfz_rate','mgwfplr','mgzbgj','mgxjllje') #zgb 一定在开头为‘mg’的前面，因为‘mg’的二次更新需要有zgb
        tablename = "select_stock_filter_%d"%year
        sql_str="UPDATE %s SET %s "%(tablename,expression)
        cur239.execute(sql_str)
    secu_list = Secu_list()  #get all stock code
    # secu_list=secu_list[:50]
    date_list, y2m = Date_list()  #get all date,from 1990-now,y2m==year:(min,max),first and last day every year,it is dic
    deal_sql(list_att)
    cnn.close()
    cur239.close()
