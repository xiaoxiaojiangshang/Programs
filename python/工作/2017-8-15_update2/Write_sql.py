...
def deal_sql(list_att):
    for key in list_att:
        ...
        if key=='zgb':
            dict_221_zgb=dict_221
            dict_121_zgb=dict_121   ##save
            ...
            if secu_id in dict_121:            #有的股票某个属性全都为空
                ...
                for index,up_day in enumerate(list_update):
                    if index==len(list_update)-1:
                        ...
                        year = 2017  # last year
                        day_end=y2m[year][1]
                        Add_list(dict_list_all, day_start,day_end,secu_id,key,dict_221[(secu_id,up_day)],dict_121_zgb,dict_221_zgb)
                    else:
...
def Add_list(dict_list_all,day_start,day_end,id_value,name,value,mg_dict_121,mg_dict_221):
    if day_start<y2m[2017][1]:     #17378=2017-7-31 end_date
        index1=return_index(day_start)+1
        index2=return_index(day_end)
        if name[0] != 'm':  # repsent mg
            for days in date_list[index1:index2 + 1]:
                one_dict={'days':days,'id':id_value,name:value}
                dict_list_all+=[one_dict]
        else:
            start_zgb = return_zgb(id_value, date_list[index1], mg_dict_121, mg_dict_221)
            end_zgb = return_zgb(id_value, date_list[index2], mg_dict_121, mg_dict_221)
            if start_zgb==end_zgb:     #same ,don't need uprate
                for days in date_list[index1:index2 + 1]:
                    one_dict = {'days': days, 'id': id_value, name: value}
                    dict_list_all += [one_dict]
            elif start_zgb:
                value_temp=value
                for days in date_list[index1:index2 + 1]:
                    days_zgb=return_zgb(id_value,days,mg_dict_121,mg_dict_221)
                    if start_zgb!=days_zgb and days_zgb:
...
def return_zgb(secu_id,days,dic_121,dic_221):
    if secu_id in dic_121:
        for day_temp in dic_121[secu_id]:
            if day_temp<days:
                continue
            else:
                return dic_221[(secu_id,day_temp)]
    else:
        return None
